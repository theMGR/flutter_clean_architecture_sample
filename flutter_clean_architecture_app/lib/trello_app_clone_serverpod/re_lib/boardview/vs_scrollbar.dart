...
class VsScrollbarStyle {
  ...
  const VsScrollbarStyle({this.radius = _kScrollbarRadius, this.thickness = _kScrollbarThickness, this.hoverThickness = _kScrollbarThicknessWithTrack, this.color});
}
...
class VsScrollbar extends StatefulWidget {
  ...
  @override
  State<VsScrollbar> createState() => _ScrollbarState();
}
...
class _MaterialScrollbarState extends RawScrollbarState<_MaterialScrollbar> {
  ...
  WidgetStateProperty<Color> get _thumbColor {
    ...
    return WidgetStateProperty.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.dragged)) return _scrollbarTheme.thumbColor?.resolve(states) ?? dragColor;

      // If the track is visible, the thumb color hover animation is ignored and
      // changes immediately.
      if (states.contains(WidgetState.hovered) && _showTrackOnHover) return _scrollbarTheme.thumbColor?.resolve(states) ?? hoverColor;
  ...
  WidgetStateProperty<Color> get _trackColor {
    ...
    return WidgetStateProperty.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.hovered) && _showTrackOnHover) {
        return _scrollbarTheme.trackColor?.resolve(states) ?? onSurface.withOpacity(0.05);
      }
  ...
  WidgetStateProperty<Color> get _trackBorderColor {
    ...
    return WidgetStateProperty.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.hovered) && _showTrackOnHover) {
        return _scrollbarTheme.trackBorderColor?.resolve(states) ?? onSurface.withOpacity(0.1);
      }
      return const Color(0x00000000);
    });
  }
  ...
  WidgetStateProperty<double> get _thickness {
    return WidgetStateProperty.resolveWith((Set<WidgetState> states) {
      if (states.contains(WidgetState.hovered) && _showTrackOnHover) return widget.hoverThickness ?? _scrollbarTheme.thickness?.resolve(states) ?? _kScrollbarThicknessWithTrack;
