...
class BoardView extends StatefulWidget {
  ...
  BoardView({super.key, this.itemInMiddleWidget, this.scrollbar, this.scrollbarStyle, this.boardViewController, this.dragDelay = 300, this.onDropItemInMiddleWidget, this.isSelecting = false, this.lists, this.width = 280, this.middleWidget, this.bottomPadding});
...
class BoardViewState extends State<BoardView> with AutomaticKeepAliveClientMixin {
  ...
  void moveListRight() {
    ...
    if (boardViewController.hasClients) {
      ...
      boardViewController.animateTo(draggedListIndex! * widget.width, duration: Duration(milliseconds: 400), curve: Curves.ease).whenComplete(() {
  ...
  void moveListLeft() {
    ...
    if (boardViewController.hasClients) {
      int? tempListIndex = draggedListIndex;
      boardViewController.animateTo(draggedListIndex! * widget.width, duration: Duration(milliseconds: widget.dragDelay), curve: Curves.ease).whenComplete(() {
        RenderBox object = listStates[tempListIndex!].context.findRenderObject() as RenderBox;
        Offset pos = object.localToGlobal(Offset.zero);
        leftListX = pos.dx;
        rightListX = pos.dx + object.size.width;
  ...
  Widget build(BuildContext context) {
    ...
    if (boardViewController.hasClients) {
      WidgetsBinding.instance.addPostFrameCallback((Duration duration) {
        try {
          ...
          debugPrint('BoardView e: $e');
        }
        bool shown_ = boardViewController.position.maxScrollExtent != 0;
        if (shown_ != shown) {
          setState(() {
            shown = shown_;
          });
    ...
    if (widget.scrollbar == true) {
      listWidget = VsScrollbar(
          ...
          style: widget.scrollbarStyle != null ? VsScrollbarStyle(hoverThickness: widget.scrollbarStyle!.hoverThickness, radius: widget.scrollbarStyle!.radius, thickness: widget.scrollbarStyle!.thickness, color: widget.scrollbarStyle!.color) : VsScrollbarStyle(),
          child: listWidget);
    }
      ...
      if (canDrag && dxInit != null && dyInit != null && !isInBottomWidget) {
        if (draggedItemIndex != null && draggedItem != null && topItemY != null && bottomItemY != null) {
          ...
          if (0 <= draggedListIndex! - 1 && dx! < leftListX! + 45) {
            //scroll left
            if (boardViewController.hasClients) {
              boardViewController.animateTo(boardViewController.position.pixels - 5, duration: Duration(milliseconds: 10), curve: Curves.ease);
              if (listStates[draggedListIndex!].mounted) {
          ...
          if (dy! < topListY! + 70) {
            ...
            if (listStates[draggedListIndex!].boardListController.hasClients && !isScrolling) {
              ...
              listStates[draggedListIndex!].boardListController.animateTo(listStates[draggedListIndex!].boardListController.position.pixels - 5, duration: Duration(milliseconds: 10), curve: Curves.ease).whenComplete(() {
...
//                }
          ...
          if (widget.middleWidget != null) {
            if (_middleWidgetKey.currentContext != null) {
              RenderBox box_ = _middleWidgetKey.currentContext!.findRenderObject() as RenderBox;
              tempBottom = box_.size.height;
...
//                }
          ...
          if (widget.lists![draggedListIndex!].items!.length > draggedItemIndex! + 1 && dy! > bottomItemY! + listStates[draggedListIndex!].itemStates[draggedItemIndex! + 1].height / 2) {
            //move down
            moveDown();
          }
        } else {
